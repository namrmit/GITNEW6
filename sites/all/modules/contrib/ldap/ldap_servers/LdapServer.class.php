...
class LdapServer {
  ...
  function bind($userdn = NULL, $pass = NULL, $anon_bind = FALSE) {
    //watchdog('ldap',  $userdn);
    //watchdog('ldap',  print_r($this,true));
    ...
    if ($anon_bind === TRUE) {
      ...
      //if ($userdn === NULL || $pass === NULL) return LDAP_LOCAL_ERROR;
      
      $userdn = ($userdn != NULL) ? $userdn : $this->binddn;
      $pass = ($pass != NULL) ? $pass : $this->bindpw;
      
      $userdn = str_replace(array(chr(0), '%00', '%2500'), '', $userdn);
      $pass = str_replace(array(chr(0), '%00', '%2500'), '', $pass);  
      
      if(drupal_strlen($userdn) == 0 || drupal_strlen($pass) == 0) return LDAP_CONNECT_ERROR;
